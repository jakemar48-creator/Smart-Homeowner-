...
export default function StartPage() {
  ...
  useEffect(() => {
    ...
    // Campaign links can use either /start?partner=roof-depot or the clean
    // partner route, such as /roof-depot. Both use this same funnel.
    const pathPartner = window.location.pathname.split('/').filter(Boolean)[0];
    setPartner(params.get('partner') ?? (pathPartner !== 'start' && pathPartner !== 'thankyou' ? pathPartner : ''));
